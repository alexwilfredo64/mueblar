TO DO

# Modificaciones en Figma
## App
- Coleccion por defecto sera **Favoritos**
- Corregir materiales: Separando en viñetas donde `Materiales` es el titulo.
- Agregar Selector de colores/variantes en vista de detalles del producto entre el tiulo y las imagenes
- Vista de coleccion pasa a ser pantalla independiente (en lugar de Perfil)
- **COMPLETADO** Cambiar boton color a trasladar (Tipo zoom de camara pero con funcionalidad de alejar o acercar el objeto en el eje frontal)

## Web
- Vista de coleccion se refiere a catalogo de categorias
- Agregar Vista de colecciones del usuario
- Agregar modulo de detalles de producto Excluyendo lo relacionado al AR
- Separar flujo de Administrador (Usuarios, Roles, productos, categorias, auditoria) y gerente de tienda (dashboard, inventario, informes)
- Vista de perfil de usuario
- Quitarle el `Agregar nuevo producto al Gerente`
- Cambiar la vista de crear productos a vista de variaciones (no incluye descripcion, tienda ni dimensiones)
- Agregar vista de modelo (Un modelo/producto solo se compone de nombre, tienda, descripcion general, y dimensiones)
